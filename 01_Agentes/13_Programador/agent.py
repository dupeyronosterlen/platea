...
GTM_ID             = "GTM-P4BDXRN9"
GA4_ID             = "G-NXF8093MDJ"
TEATRO_ID          = "wilberto"  # tid canónico del Worker/KV — ver produccion.yaml → temporada.venue.tid
...
ENDPOINTS = [
    ...
    {"nombre": "Worker API",        "url": f"{BOLETERA_URL}/api/{TEATRO_ID}/funciones", "metodo": "GET", "espera": 200},
    {"nombre": "/api/disponibilidad","url": f"{BOLETERA_URL}/api/{TEATRO_ID}/disponibilidad?fecha={datetime.date.today().isoformat()}", "metodo": "GET", "espera": 200},
...
def build_health_report(health: dict) -> str:
    ...
    if caidos:
        alertas = "\nALERTAS\n" + "─" * 60 + "\n"
        for c in caidos:
            detalle = c.get("error") or f"HTTP {c['status']}"
            alertas += f"  ❌ {c['nombre']}: {detalle}\n"
            alertas += f"     URL: {c['url']}\n"
