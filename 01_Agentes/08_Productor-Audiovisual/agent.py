...
ENV_PATH = Path(__file__).resolve().parents[2] / "privado" / "credenciales" / ".env"
load_dotenv(ENV_PATH)

IG_USER_ID  = os.getenv("IG_USER_ID", "17841405953353649")
GCP_PROJECT = os.getenv("GCP_PROJECT", "agencia-mkt-ia")

# ─── CONFIG DE PRODUCCIÓN (fuente única: 03_Producciones/<produccion-activa>/produccion.yaml (puntero en config/produccion-activa.txt)) ──────
...
def _fmt_fecha_corta(d: datetime.date) -> str:
    return f"{d.day} {MESES_ES[d.month]} {d.year}"


CONFIG = cargar_config()
_venue = CONFIG["temporada"]["venue"]
_precios = CONFIG["precios"]
_funciones_iso = CONFIG["temporada"]["funciones"]
_horario = CONFIG["temporada"]["horario"]
...
NARRATIVA = {
    ...
    "venue": f"{_venue['nombre']}, CDMX",
    "fechas": (
        f"Sábados {_horario}h — "
        f"{_fmt_fecha_corta(datetime.date.fromisoformat(_funciones_iso[0]))} a "
        f"{_fmt_fecha_corta(datetime.date.fromisoformat(_funciones_iso[-1]))}"
    ),
    "precio": f"${_precios['general']} general / ${_precios['descuento_academia']} estudiantes",
}
...
FUNCIONES_S2 = [datetime.date.fromisoformat(f) for f in _funciones_iso]
...
REGLAS DE CONTENIDO
  ✓ Precio SIEMPRE visible en posts de conversión (${_precios['general']} / ${_precios['descuento_academia']} est.)
...
CALENDARIO DE FUNCIONES ({len(FUNCIONES_S2)} sábados {_horario}h)
