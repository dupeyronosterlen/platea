...
_PRODUCCION_ACTIVA = (AGENCIA_ROOT / "config" / "produccion-activa.txt").read_text().strip()
CONFIG_FILE   = AGENCIA_ROOT / "03_Producciones" / _PRODUCCION_ACTIVA / "produccion.yaml"
...
ESTRENO_PRENSA = _TEMPORADA["estreno_prensa"]
NUM_FUNCIONES = len(_TEMPORADA["funciones"])
AFORO_TOTAL = _TEMPORADA["venue"]["aforo_total"]
AFORO_VENDIBLE = _TEMPORADA["venue"]["aforo_vendible"]
PRECIO_GENERAL = _PRECIOS["general"]
PRECIO_ACADEMIA = _PRECIOS["descuento_academia"]
CPA_MAX_ACEPTABLE = _KPIS["cpa_max_aceptable"]
CPA_REFERENCIA_S1 = _KPIS["cpa_referencia_s1"]
...
def cargar_contexto_completo() -> str:
    ...
    # Estado S2
    partes.append("\n## CONTEXTO S2 ACTIVO")
    partes.append(f"- Obra: {OBRA_NOMBRE} — Humberto Dupeyrón")
    partes.append(f"- Venue: {VENUE_NOMBRE} (SOGEM), CDMX")
    partes.append(
        f"- {NUM_FUNCIONES} funciones: {DIA_FUNCION.lower()} {HORARIO}h, "
        f"desde {ESTRENO_PRENSA} (cierre por confirmar)"
    )
    partes.append(f"- Aforo: {AFORO_TOTAL} localidades (~{AFORO_VENDIBLE} vendibles)")
    partes.append(f"- Precio: ${PRECIO_GENERAL} MXN general / ${PRECIO_ACADEMIA} MXN descuento")
    partes.append(f"- CPA objetivo: ≤ ${CPA_MAX_ACEPTABLE} MXN (medido en boletera, no en Meta/Google)")
    partes.append(f"- CPA real S1: ${CPA_REFERENCIA_S1} MXN (ads ganadores documentados)")
...
1. Los 3 riesgos más urgentes para el lanzamiento S2 ({ESTRENO_PRENSA})
