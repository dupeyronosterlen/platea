...
TEMPORADA_INICIO = datetime.date.fromisoformat(_TEMPORADA["estreno_prensa"])
TEMPORADA_CIERRE = datetime.date.fromisoformat(_TEMPORADA["temporada_cierre"])
...
NICHOS = [
    ...
    },
    {
        ...
        "razon": f"Función especial gala/beneficio sin conflicto con {VENUE_NOMBRE}",
...
# Tablero real compartido con Ag-10 (coordinación) — ver playbook.md §inputs
LISTA_FILE = ROOT / "02_Conocimiento" / "booking" / "lista-investigacion.md"
...
TEMPORADA S2 ACTIVA: {TEMPORADA_INICIO.strftime('%-d %b')} – {TEMPORADA_CIERRE.strftime('%-d %b %Y')}
Los bolos confirmados para S2 deben NO conflictuar con funciones de sábado en {VENUE_NOMBRE}.
