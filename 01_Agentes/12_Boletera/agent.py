...
def cortesias_conocidas(fecha_str: str) -> int:
    """Cortesías/altas manuales conocidas para una función (manifiesto manual — ver CORTESIAS_FILE)."""
    try:
...
# Parámetros de la producción activa (03_Producciones/<produccion-activa>/produccion.yaml (puntero en config/produccion-activa.txt))
AFORO_TOTAL     = _config["temporada"]["venue"]["aforo_total"]
AFORO_VENDIBLE  = _config["temporada"]["venue"]["aforo_vendible"]
PRECIO_GENERAL  = _config["precios"]["general"]   # MXN
ESTRENO         = datetime.date.fromisoformat(_config["temporada"]["estreno_prensa"])
HORARIO         = _config["temporada"]["horario"] + "h"
FIN_TEMPORADA   = datetime.date.fromisoformat(_config["temporada"]["temporada_cierre"])

# Umbrales de alerta (de 03_Producciones/<produccion-activa>/produccion.yaml (puntero en config/produccion-activa.txt) → kpis)
ALERTA_OCUPACION   = _config["kpis"]["ocupacion_critica_pct"]   # % — enviar propuesta si < este porcentaje
ALERTA_DIAS_ANTES  = 5      # días antes de la función
VIGILANCIA_DIAS    = 10     # monitorear con más atención si < ocupacion_vigilar_pct

FUNCIONES_S2 = [
    datetime.date.fromisoformat(f) for f in _config["temporada"]["funciones"]
]
...
def build_inventory_snapshot() -> dict:
    ...
    for fecha in FUNCIONES_S2:
        ...
        cortesias   = cortesias_conocidas(fecha_str)
        vendidos_pagados = max(0, vendidos - cortesias)
        ocupacion   = round(vendidos / total_aforo * 100, 1) if total_aforo else 0  # ocupación física (incl. cortesías) — control de aforo
        ingresos_est = vendidos_pagados * PRECIO_GENERAL  # estimado base — solo boletos pagados
    ...
    total_vendidos = sum(f["vendidos"] for f in funciones_data)  # incluye cortesías (ocupación física)
    total_cortesias = sum(f["cortesias"] for f in funciones_data)
    total_vendidos_pagados = sum(f["vendidos_pagados"] for f in funciones_data)
    ...
    return {
        ...
        "resumen": {
            ...
            "total_vendidos": total_vendidos,  # incluye cortesías — ocupación física del teatro
            "total_cortesias": total_cortesias,
            "total_vendidos_pagados": total_vendidos_pagados,  # boletos PAGADOS reales — usar este para hablar de "ventas"
...
def evaluate_alerts(snapshot: dict) -> list[dict]:
    ...
    for fn in funciones:
        ...
        # 🟡 Función próxima (<= 10 días) con ocupación entre alerta y vigilancia
        elif delta <= VIGILANCIA_DIAS and ALERTA_OCUPACION <= ocp < _config["kpis"]["ocupacion_vigilar_pct"]:
            alertas.append({
                "nivel": "🟡",
...
def build_report(snapshot: dict, alertas: list[dict]) -> str:
    ...
    # Tabla de funciones
    tabla = "\n   Fecha          Días  Pagados  Cortesías  Ocupación  Estado\n"
    tabla += "   " + "─" * 65 + "\n"
    for fn in snapshot.get("funciones", []):
        ...
        tabla += (
            ...
            f"{fn['vendidos_pagados']:>5}/{fn['total_aforo']:<6}  "
            f"{fn['cortesias']:>9}  "
...
╔══════════════════════════════════════════════════════════════════╗
║          AGENTE 12 — BOLETERA SYNC                              ║
║          {_config["obra"]["nombre"]} · {_config["temporada"]["venue"]["nombre"]} · Sábados {HORARIO}
...
─────────────────────────────────
Funciones totales:  {resumen.get('total_funciones', 0)} ({_config["temporada"]["nombre"]} · {HORARIO})
Funciones pasadas:  {resumen.get('funciones_pasadas', 0)}
Funciones activas:  {resumen.get('funciones_activas', 0)}
Vendidos (PAGADOS): {resumen.get('total_vendidos_pagados', 0)} boletos  ← usar este número para hablar de ventas/CPA
Cortesías conocidas:{resumen.get('total_cortesias', 0)} boletos (prensa/invitados — NO cuentan como venta)
Ocupación física:   {resumen.get('total_vendidos', 0)} boletos en butaca (pagados + cortesías)
Ingresos est.:      ${resumen.get('total_ingresos_estimados', 0):,.0f} MXN (solo boletos pagados)
Ocupación promedio: {resumen.get('ocupacion_promedio', 0):.1f}% (funciones activas, física)
...
def main():
    ...
    if not as_json:
        print(f"🎟️  Agente 12 — Boletera Sync ({_config['obra']['nombre']})")
        print("=" * 50)
        print(f"   Hoy: {hoy}")
    ...
    if nivel_urgente:
        send_alert_email(
            f"[URGENTE] Boletera {_config['obra']['nombre']} — función {fecha_prox}",
            report,
            urgente=True,
        )
    elif nivel_alerta:
        send_alert_email(
            f"[Vigilancia] Boletera {_config['obra']['nombre']} — {hoy.strftime('%d %b')}",
            report,
            urgente=False,
        )
    else:
