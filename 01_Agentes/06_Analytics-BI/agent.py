...
_PRODUCCION_ACTIVA = (AGENCIA_DIR / "config" / "produccion-activa.txt").read_text().strip()
CONFIG_FILE = AGENCIA_DIR / "03_Producciones" / _PRODUCCION_ACTIVA / "produccion.yaml"
...
# Cliente Gemini vía wrapper (AI Gateway si CF_AI_GATEWAY_BASE está seteado)
sys.path.insert(0, str(BASE_DIR.parent / "_lib"))
...
AD_ACCOUNT_ID = os.environ.get("AD_ACCOUNT_ID", _config["ads"]["meta"]["ad_account_id"])
GOOGLE_ADS_CUSTOMER_ID = _config["ads"]["google"]["ads_customer_id"]
...
# ── Cliente Vertex AI (Cloudflare AI Gateway si hay CF_AI_GATEWAY_BASE) ────────
client = make_client(project=GCP_PROJECT, location=GCP_LOCATION)

# ── Umbrales (de 03_Producciones/<produccion-activa>/produccion.yaml (puntero en config/produccion-activa.txt) → kpis) ────────────────────────
CPA_LIMITE = _config["kpis"]["cpa_max_aceptable"]      # MXN — alerta roja
CPA_MONITOREO = _config["kpis"]["cpa_target"]          # MXN — alerta amarilla
OCUPACION_ALERTA = _config["kpis"]["ocupacion_vigilar_pct"] / 100
OCUPACION_URGENTE = _config["kpis"]["ocupacion_critica_pct"] / 100
