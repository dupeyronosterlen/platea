...
# Usa el .env maestro (mismo que Ag-03) — el .env local de Media-Buyer no existe
ENV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "privado", "credenciales", ".env")
...
# Un solo action_type por evento — NUNCA sumar alias equivalentes del mismo
# evento (Meta reporta el mismo Purchase/InitiateCheckout bajo varios nombres
# a la vez; sumarlos triplica el conteo, verificado empíricamente 16 ago 2026).
# Prioridad: el más nuevo/unificado primero, con fallback si Meta lo omite.
ACTION_PRIORITY = {
    "checkouts":     ["omni_initiated_checkout", "initiate_checkout",
                       "offsite_conversion.fb_pixel_initiate_checkout"],
    "compras_meta":  ["omni_purchase", "purchase",
...
def pick_action(actions: list, priority: list) -> tuple:
    """Un solo valor por evento: recorre 'priority' en orden y devuelve el
    primer action_type que Meta sí trajo en esta fila, junto con su nombre
    (para poder auditar qué fuente se usó, sin imprimirlo por default)."""
    disponibles = {a.get("action_type"): a.get("value") for a in (actions or [])}
    for tipo in priority:
        if tipo in disponibles:
            try:
                return int(float(disponibles[tipo])), tipo
            except (TypeError, ValueError):
                continue
    return 0, None
...
def fetch_meta_funnel(days: int) -> dict:
    ...
    for row in rows:
        ...
        for stage, priority in ACTION_PRIORITY.items():
            c[stage], c[f"{stage}_fuente"] = pick_action(row.get("actions"), priority)
        campanas.append(c)
        tot["spend"] += c["spend"]
...
def build_text(out: dict) -> str:
    ...
    try:
        if str(ops) not in sys.path:
            sys.path.insert(0, str(ops))
        from modelo_diario_text import format_modelo_email_block
        modelo_block = format_modelo_email_block() + "\n\n"
    except Exception:
        pass
