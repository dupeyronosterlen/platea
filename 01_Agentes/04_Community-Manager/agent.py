...
DM protocol:
    - Responder <2h
    - Grupos/familias → dar link + código ESPEJO
...
- Precio pareja: ${precios.get('espejo')} MXN (código ESPEJO)
- Descuento académico: ${precios.get('descuento_academia')} MXN (tarifa credencial, no cupón ACADEMIA)
...
- Si preguntan por grupos o parejas → dar link + mencionar ESPEJO
...
{{
  ...
  "codigo_incluir": "ESPEJO si aplica, o null",
...
OBRA: {obra.get('nombre')} | {obra.get('actor_principal')}
URL BOLETOS: {obra.get('url_boletos')}
PRECIO: ${precios.get('general')} MXN | ESPEJO: ${precios.get('espejo')} la pareja
